...
- `update_state(state:Dictionary)` with coins:int, keep_health:float, keep_max:float, wave:int, total_waves:int, wave_text:String, wave_active:bool, wave_remaining:int, wave_total:int, kills:int, hero_level:int, xp:float, next_xp:int, ability_unlocked:bool, ability_cooldown:float, ability_total:float, threat_text:String. Remaining enemies includes both pending spawns and surviving actors; the completion bar advances on kills.
