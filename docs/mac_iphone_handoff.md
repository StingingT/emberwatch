...
The current Windows checkpoint contains six playable campaign missions, saved results/unlocks, persistent settings, tutorial guidance and an ending. It passed 354 checks and legal playthroughs of all six missions, including a fresh source extraction; see [campaign_validation.md](campaign_validation.md). Native Mac/iPhone validation remains a later or parallel workstream and does not block continuing desktop development.
...
3. Run the README's headless checks, including profile/campaign checks and both playthrough scripts. The current checkpoint expects 354 checks plus successful original and six-mission campaign scenarios. Require the printed success markers and no script errors. These tests use isolated or memory-only profiles.
...
The profile is `user://emberwatch_profile.json` inside the app's Godot user-data location, with a previous-valid `.bak`. Corruption/future-version/write-failure checks belong in isolated test paths; preserve actual player saves. Verify visible recovery/save-failure notices on native builds. The current checkpoint does **not** restore a partly completed battle after process termination: interrupted-run recovery is the next implementation step, distinct from preserving completed missions and settings.
...
| Partly completed battle after process termination | Recovery implementation next; not a completed checkpoint claim |
