...
Hunter actor implementation: wolf-pelt silhouette with paired blades, bounded pursuit within 7 units of the hero and 5 units of its current route segment, and existing telegraphed melee. Barricades block pursuit. Targeted checks verify pursuit, leash refusal and abandoning a dead hero. It is placed in campaign waves as described below; human encounter tuning remains pending.
...
Piercing projectile foundation is implemented: a fixed-line spear-like arrow crosses up to three enemies, uses swept intersections in near-to-far order, and retains per-target exclusions across an interrupted run. Damage grants proportional hero XP. Ten targeted headless checks and a portrait rendering passed. It is now selectable through the Piercing level-up choice.

Choice integration: levels 2-6 pause the battle and offer Multishot, Volley or Piercing. Each point adds two secondary 55%-damage arrows, adds two Volley targets and 20% base burst damage, or enables an 85%-damage piercing primary with one additional penetration per later rank. Choices combine and remain through death; they reset with a new run. Held touch input is cleared and held keyboard movement requires release. Fourteen targeted headless checks plus portrait UI inspection passed; all six campaign simulations pass with choices. Human balance and spectacle acceptance remain pending.

Building silhouette pass: tower tiers now rise from timber to taller masonry to a broad paired-launcher platform; L3 walls gain red-roofed end towers; upgraded mines expose winding machinery; the L2 Smith has a broad flat workshop canopy between the L1 cottage and L3 twin-stack forge. The 12-model gallery and per-building normal portrait camera captures were generated and inspected. Ground footprints remain governed by the existing collision data. Human tier-recognition acceptance remains pending. The subsequent floating-bulb layout below replaces the old controls that obscured upper geometry.

Smith bulbs: circular, individually colored purchase controls now show an effect symbol, concise benefit and live gold price. Their layout follows the forge anchor, keeps the central forge/hero visible with a left bulb and right pair where space permits, and falls back to a single separated row on compact views. Disabled and pressed appearances remain distinct. All 137 UI checks passed, including touch/context refresh and safe-area checks; the revised L3 forge portrait was inspected. Human movement-plus-purchase playtesting is still pending.

Feedback integration: lethal impacts now add expanding rings and shards, gold collection adds a short sparkle around the accurate pickup total, coins emerge with a higher bounce, and actual build/upgrade completion triggers an expanding construction ring with sparks. Construction shares the existing 32-effect budget; reduced motion suppresses these motion effects. Seventeen feedback checks and 71 actor snapshot checks pass; portrait feedback captures were inspected. These are implemented effects, not human spectacle acceptance: normal-speed busy combat and first-three-mission playtest feedback remain required.

## Current acceptance boundary

The 0.4.0 Android debug build and source review ZIP are available with docs/playtest_040.md. Automated and staged-rendering evidence establishes implementation, not milestone acceptance. No Android device was attached when checked with adb after packaging. Await first-three-mission human feedback and device observations before reassessment; the HOLD list remains in force.
